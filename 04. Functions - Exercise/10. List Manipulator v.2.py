def is_valid_index(collection: list, index):
...
def swap_halfs_at_index(collection, index):
    swapped_list = []

    if not is_valid_index(collection, index):
        print("Invalid index")
        swapped_list = collection
    else:
        half_1 = collection[:index + 1]
        half_2 = collection[index + 1:]
        swapped_list = half_2 + half_1
...
def max_num_by_criteria(collection: list, custom_filter):
    ...
    max_element_index = -1 # retuns -1 if there is no match
...
def min_num_by_criteria(collection: list, custom_filter):
    min_element = float("inf") # change to sys.maxsize if this does not work
    min_element_index = -1
...
def first_count_elements_by_criteria(collection, count, custom_filter):
...
def last_count_elements_by_criteria(collection: list, count, custom_filter):
    if count > len(collection):
        print("Invalid count")
    else:
        matching_elements = []
...
def parse_collection_to_int(collection: list):
    ...
    for element in collection:
        parsed_collection.append(int(element))

    return parsed_collection


def stringify_collection(collection: list, delimeter: str):
    stringified_collection = []

    for el in collection:
        stringified_collection.append(str(el))

    return delimeter.join(stringified_collection)
...
while not command == "end":
    ...
    if cmd_type == "exchange":
        ...
        numbers = swap_halfs_at_index(numbers, index)
